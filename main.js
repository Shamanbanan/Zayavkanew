...
if (data.status === "Оприходован") {
  document.querySelectorAll("disable-on-checkbox").disabled = true;
}else{
  document.querySelectorAll("disable-on-checkbox").enabled = true;
};
  ...
  item.innerHTML = `
    ...
    ${data.code ? `<span id="status">${data.status}</span>` : `<span id="status">Обработка</span>`}
    <svg class="disable-on-checkbox" id="readct" width="800px" height="800px" viewBox="0 0 24 24" fill="gray" xmlns="http://www.w3.org/2000/svg">
  ...
  item.querySelector("#readct").addEventListener("click", () => {
    ...
    modal.innerHTML = `
        <div class="modal-body">
          ...
          <label for="categoryModal">Категория</label><input id="categoryModal" value="${data.category}">
          <label for="nameModal">Наименование</label><input id="nameModal" value="${data.name}">
          <label for="variationModal">Вариант исполнения</label><input id="variationModal" value="${data.variation}">
          <label for="typeModal">Баз.ед.</label><input id="typeModal" value="${data.type}">
          <label for="dequipmentModal">Оборудование</label><input id="equipmentModal" value="${data.equipment}">
          <label for="articleModal">Статья</label><input id="articleModal" value="${data.article}">
          <label for="userIdModal">ФИО</label><input id="userIdModal" value="${data.userId}">
          <label for="codeModal">Код</label><input id="codeModal" type="number" value="${data.code}">
          <select id="status">
    ...
    // Save data to Firebase when btnSave is clicked
